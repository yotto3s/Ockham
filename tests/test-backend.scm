...
(test-group "be:call-core-integration"
  (let* ((op-call-sexp '(%res : (int 32) = (be:call $fib %a)))
    ...
    (test-equal '$fib (call-callee (operation-op op)))
...
(test-group "be:func-serialization"
  (let* ((i32 (make-int 32))
         ...
         (f2 (make-func '$div_mod (list (cons '%a i64) (cons '%b i64)) (list i64 i64) reg))
         (s2 (func-serialize f2))
         (d2 (func-deserialize s2))
         (d3 (func-deserialize '(be:func $fib ((%a : (int 32))) -> (int 32) (region (block ^bb0)))))
         (f4 (make-func '$alloc (list (cons '%sz i64)) (list (make-ptr)) reg))
         (s4 (func-serialize f4))
         (d4 (func-deserialize s4)))
    (test-equal '(be:func $fib ((%a : (int 32))) -> ((int 32)) (region (block ^bb0))) s1)
    (test-equal '(be:func $div_mod ((%a : (int 64)) (%b : (int 64))) -> ((int 64) (int 64)) (region (block ^bb0))) s2)
    (test-equal '(be:func $alloc ((%sz : (int 64))) -> (ptr) (region (block ^bb0))) s4)
...
(test-group "be:func-core-integration"
  (let* ((op-func-sexp '((be:func $fib ((%a : (int 32))) -> ((int 32)) (region (block ^bb0)))))
    ...
    (test-equal '$fib (func-name (operation-op op)))
    (test-equal op-func-sexp (operation-serialize op))))

(test-group "be:global-int-serialization"
  (let* ((gi (make-global-int '$foo 32 42))
         (s (global-int-serialize gi))
         (d (global-int-deserialize s))
         (d2 (global-int-deserialize '(be:global.int $foo 32 42))))
    (test-equal '(be:global-int $foo 32 42) s)
    (test-assert (global-int? d))
    (test-equal '$foo (global-int-name d))
    (test-equal 32 (global-int-bit-width d))
    (test-equal 42 (global-int-init-value d))
    (test-assert (global-int? d2))
    (test-equal 42 (global-int-init-value d2))))
...
(test-group "be:global-int-core-integration"
  (let* ((op-sexp '((be:global-int $val 64 100)))
    ...
    (test-equal '$val (global-int-name (operation-op op)))
    (test-equal 64 (global-int-bit-width (operation-op op)))
    (test-equal 100 (global-int-init-value (operation-op op)))
    (test-equal op-sexp (operation-serialize op))))

(test-group "be:global-bytes-serialization"
  (let* ((gb1 (make-global-bytes '$str "hello, world"))
         (s1 (global-bytes-serialize gb1))
         (d1 (global-bytes-deserialize s1))
         (gb2 (make-global-bytes '$raw "data" #f))
         (s2 (global-bytes-serialize gb2))
         (d2 (global-bytes-deserialize s2))
         (d3 (global-bytes-deserialize '(be:global.bytes $msg "test"))))
    (test-equal '(be:global-bytes $str "hello, world") s1)
    (test-equal '(be:global-bytes $raw "data" #f) s2)
    (test-assert (global-bytes? d1))
    (test-equal '$str (global-bytes-name d1))
    (test-equal "hello, world" (global-bytes-data d1))
    (test-assert (global-bytes-null-terminate? d1))
    (test-assert (global-bytes? d2))
    (test-assert (not (global-bytes-null-terminate? d2)))
    (test-assert (global-bytes? d3))
    (test-equal "test" (global-bytes-data d3))))
...
(test-group "be:global-bytes-core-integration"
  (let* ((op-sexp '((be:global-bytes $msg "hello")))
    ...
    (test-equal '$msg (global-bytes-name (operation-op op)))
